...
import { clearRefreshToken, loadRefreshToken, OCA_CONFIG, saveRefreshToken } from "./config"
...
export async function exchangeCodeForTokens(
	...
	const discovery = await axios.get(`${OCA_CONFIG.idcs_url}/.well-known/openid-configuration`)
...
export class TokenManager {
	...
	async getToken(): Promise<string> {
		if (!this.refreshToken) {
			throw new Error("Not authenticated. Please visit /login to authenticate with OCA.")
		}
		...
		if (this.accessToken && this.tokenExpiry) {
			...
			if (timeUntilExpiry > 300) {
				log.debug(`Using cached access token (expires in ${Math.floor(timeUntilExpiry)}s)`)
	...
	private async doRefreshToken(): Promise<string> {
		...
		try {
			...
			if (!refresh) throw new Error("Not authenticated. Missing refresh token.")
			const params = new URLSearchParams({
			...
			log.success(`Successfully refreshed access token (expires in ${expiresIn}s)`)
			return this.accessToken || ""
		} catch (error: unknown) {
			...
			log.error(`Failed to refresh access token: ${status} ${JSON.stringify(data)}`)

			// Check if refresh token is invalid
			const dataObj = (data ?? {}) as { error?: string }
			if (status === 400 && (dataObj.error === "invalid_grant" || dataObj.error === "invalid_token")) {
				log.error("Refresh token is invalid/expired. Clearing authentication.")
				this.clearAuth()
				throw new Error("Refresh token expired. Please visit /login to re-authenticate.")
			}

			throw new Error(`Failed to refresh OCA access token: ${err.message || "unknown error"}`)
		}
	}
